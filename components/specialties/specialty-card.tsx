...
export function SpecialtyCard({ specialty, matchPercentage }: { specialty: SpecialtyProfile; matchPercentage?: number }) {
  return (
    <Card className="group h-full" role="article" aria-label={`${specialty.name} specialty card`}>
      ...
      <div className="mt-6 flex items-center justify-between">
        <div className="text-sm text-foreground/60">
          {matchPercentage ? (
            <span aria-label={`${matchPercentage} percent match`}>{matchPercentage}% match</span>
          ) : (
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 text-sm font-semibold text-primary transition group-hover:gap-3 focus-visible:ring-2 focus-visible:ring-primary/70 focus-visible:ring-offset-2 rounded-sm"
