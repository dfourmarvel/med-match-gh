...
- **TEST-2** — expanded `collectCoverageFrom` to `lib/**` + `app/api/**` (excluding the LLM wrapper/prompt strings, the deprecated adapter, and the seed-only `lib/scoring/`), and set per-metric thresholds as a regression floor (statements 65 / branches 55 / functions 72 / lines 65). Added meaningful tests: `lib/__tests__/scoring.test.ts` (production scorer — trait scoring, ranking, schema-contract), `components/quiz/__tests__/assessment-client.test.tsx` (A11Y-1 keyboard radios), `app/api/score/__tests__/route.test.ts`, and — with Supabase + rate-limit mocked — `lib/__tests__/results.test.ts` plus route tests for `save-result`, `quiz-results`, and `results/[id]`. **50 tests**; coverage ≈ **69% statements / 60% branches / 80% functions / 70% lines**. `npm test` and `npm run test:coverage` both pass.
...
Every phase in the fix order has been executed, verified, committed, and pushed to `main`. Remaining known follow-ups (out of the original audit scope): set `NEXT_PUBLIC_SITE_URL` and the Supabase/Groq/Upstash env vars in Vercel (see [PRODUCTION_SETUP.md](PRODUCTION_SETUP.md)); optionally add a Content-Security-Policy (deferred from SEC-5).

**Post-audit addition (2026-07-13):** DB-backed API route tests (save-result, quiz-results, results/[id]) and `lib/results.ts` tests added with Supabase mocked; suite grew to 50 tests and coverage to ≈70% lines, so the coverage floor was raised (statements 65 / branches 55 / functions 72 / lines 65).
